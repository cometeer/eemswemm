...
export function UnityObject2(K) {
  ...
  var t = function() {
    var al = Y.userAgent, an = Y.platform;
    ...
    var aq = {
      ...
      edge: /edge/i.test(al) ? parseFloat(al.replace(/^.*edge\/(\d+(\.\d+)?).*$/i, "$1")) : false
    };
    aq.clientBrand = aq.edge ? "edge" : aq.ch ? "ch" : aq.ff ? "ff" : aq.sf ? "sf" : aq.ie ? "ie" : aq.op ? "op" : "??";
  ...
  var j = function() {
    ...
    function an() {
      ...
      ak.push(["unity._setCustomVar", 1, "Revision", "fdbb5133b820", 2])
    }
    ...
    function am(av, at, aw, ar) {
      ...
      aq += "&v=" + encodeURIComponent("fdbb5133b820");
      if (y.referrer !== null) {
        aq += "?r=" + y.referrer
      }
      if (at) {
  ...
  function g(an) {
    var ao = C[an], am = aa.getElementById(an), al;
    if (!am) {
      return
    }
  ...
  var S = {
    ...
    installPlugin: function(ak) {
      ...
      switch (ak) {
        ...
        case"ClickOnceIE":
          u = true;
          e(r, u);
          var al = $("<iframe src='" + this.getClickOnceURL() + "' style='display:none;' />");
          $(y.targetEl).append(al);
          break;
        default:
        case"Manual":
          var al = $("<iframe src='" + this.getPluginURL() + "' style='display:none;' />");
          $(y.targetEl).append(al);
          break
      }
      Q = ak;
    ...
    trigger: function(ak, aj) {
      ...
      $(document).trigger(ak, aj)
    },
    displayDeprecatedBanner: function(ak, al) {
      var aj = aa.getElementById("ThisBrowserDoesNotSupportUnityPlayer");
      if (!aj) {
        var am = $("<div id='ThisBrowserDoesNotSupportUnityPlayer'>");
        am.css("width", "100%").css("height", "200px").css("background", "#f9f8e6").css("background", "rgba(231,228,157,.25)").css("border-color", "#fcfbf1").css("border-color", "rgba(231,228,157,.15)").css("border-style", "solid").css("border-width", "5x").css("color", "#595959").css("color", "rgba(0,0,0,.65)").css("text-align", "left").css("padding", "10px").appendTo(ak);
        var an = "<img src='https://files.unity3d.com/UnityObject2/resources/other_browser.jpg' style='float: left; margin-right: 15px;' /><div style='overflow:hidden;'><div style='height:202px; display:inline-block; vertical-align:middle;'></div><div style='display:inline-block;'><div style='display:inline-block; vertical-align:middle;'>" + al + "</div></div></div>";
        am.html(an)
      }
    },
    displayCantRunBanner: function(aj, ak, am) {
      var al = "<span style='font-weight:bold; font-size: 1.1em;'>Sorry, " + ak + " can't run this app</span><p>You are using " + ak + " that does not support the Unity Web Player plugin needed to run this app.<br/>We recommend using another browser, such as " + am + ".</p>";
      this.displayDeprecatedBanner(aj, al)
    },
    notifyProgress: function(ak) {
      if (typeof ac !== "undefined" && typeof ac === "function") {
        ...
        if (V === M) {
          if (t.edge) {
            al.pluginStatus = w;
            var aj = "<a href='http://windows.microsoft.com/en-us/internet-explorer/'>Internet Explorer</a>, <a href='http://www.mozilla.org/firefox'>Firefox</a> or <a href='http://www.opera.com/'>Opera</a>";
            this.displayCantRunBanner(ak, "Microsoft Edge", aj)
          } else {
            var am = t.ch && (t.ch_v > 41);
            if (am && y.enableBrowserDeprecatedWarning) {
              al.pluginStatus = w;
              var aj = "";
              if (t.win) {
                aj = "<a href='http://windows.microsoft.com/en-us/internet-explorer/'>Internet Explorer</a>, <a href='http://www.mozilla.org/firefox'>Firefox</a> or <a href='http://www.opera.com/'>Opera</a>"
              } else {
                aj = "<a href='http://www.mozilla.org/firefox'>Firefox</a>, <a href='https://www.apple.com/safari/'>Safari</a>"
              }
              if (t.ch_v < 45) {
                var an = "<span style='font-weight:bold; font-size: 1.1em;'>Sorry, Chrome can't run this app</span><p>You are using a version of Chrome that does not currently support the Unity Web Player plugin needed to run this app.<br/>We recommend using another browser, such as " + aj + '.<br/>Alternatively, you can enable NPAPI plugins at chrome://flags/#enable-npapi (requires browser relaunch).</p><p>If you enabled the NPAPI flag and the plugin still does not work for you, try <a href="' + this.getPluginURL() + '">manual install</a>.</p>';
                this.displayDeprecatedBanner(ak, an)
              } else {
                this.displayCantRunBanner(ak, "Google Chrome", aj)
              }
            } else {
              al.bestMethod = this.getBestInstallMethod()
            }
          }
        }
        if (f !== V) {
